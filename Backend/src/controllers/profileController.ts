...
export class ProfileController {
  ...
  static async uploadProfilePhoto(req: AuthRequest, res: Response) {
    try {
      ...
      return res.json({
        ...
        data: { profilePhotoUrl: photoUrl, thumbnailUrl },
      });
    } catch (error: any) {
      return res.status(500).json({
        success: false,
  ...
  static async getUserPosts(req: Request, res: Response) {
    try {
      ...
      const result = await UserModel.getUserPosts(
        userId,
  ...
  static async getUserDiscussions(req: Request, res: Response) {
    try {
      ...
      const result = await UserModel.getUserDiscussions(
        userId,
        parseInt(page as string),
        parseInt(limit as string)
      );
  ...
  static async getBookmarks(req: AuthRequest, res: Response) {
    try {
      ...
      const result = await UserModel.getUserBookmarks(
        userId,
        folder as string,
        type as string,
        parseInt(page as string),
  ...
  static async searchUserContent(req: AuthRequest, res: Response) {
    try {
      ...
      const result = await UserModel.searchUserContent(
        userId,
        q as string,
        type as string
      );
  ...
  static async getLikedPosts(req: AuthRequest, res: Response) {
    try {
      ...
      const result = await UserModel.getLikedPosts(
        userId,
        parseInt(page as string),
        parseInt(limit as string)
      );
  ...
  static async getLikedDiscussions(req: AuthRequest, res: Response) {
    try {
      ...
      const result = await UserModel.getLikedDiscussions(
        userId,
        parseInt(page as string),
        parseInt(limit as string)
      );
      ...
      return res.status(500).json({
        success: false,
        message: 'Error fetching liked discussions',
        error: error.message,
      });
    }
  }
