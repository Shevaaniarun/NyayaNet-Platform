...
const adaptPost = (apiPost: ApiPost): PostComponentType => ({
    ...
    // tolerant media mapping to avoid TS errors when API shape varies
    media: (apiPost.media as any[] | undefined)?.map((m: any) => {
        const url = m.mediaUrl ?? m.url ?? m.path ?? m.fileUrl ?? '';
        const type = m.mediaType ?? m.type ?? '';
        const mimeType = m.mimeType ?? m.contentType ?? type ?? '';
        const fileName = m.fileName ?? m.name ?? undefined;
        return {
            id: m.id,
            url,
            type,
