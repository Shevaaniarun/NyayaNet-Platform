...
export default function App() {
    ...
    const handleNavigation = (path: string, pushToHistory = true) => {
        ...
        const newView = viewMap[path] || 'profile';
        setCurrentView(newView);
        setCurrentPath(path);
        if (newView !== 'profile') setSelectedProfileUserId(null);
        if (newView !== 'notifications') setTimeout(() => refetchNotificationCount(), 1000);
        if (pushToHistory) window.history.pushState({ view: newView }, '', path);
        if (newView === 'feed' || newView === 'dashboard') refreshPosts();
    };
