...
import { Home, Newspaper, Users, MessageSquare, Briefcase, Brain, MessageCircle, BookOpen, Scale, Zap, Search, Book, User } from 'lucide-react';
                ...
                {navItems.map((item) => {
                    ...
                    return (
                        <button key={item.path} onClick={() => handleNavClick(item.path)}
                            className={`w-full flex items-center space-x-3 px-4 py-3 rounded-lg transition-all relative overflow-hidden group ${isActive ? 'bg-constitution-gold/10 text-judge-ivory border-l-2 border-constitution-gold' : 'text-constitution-gold/70 hover:bg-constitution-gold/5 hover:text-judge-ivory'}`}>
                            ...
                            {item.badge && <span className="px-2 py-0.5 bg-constitution-gold text-justice-black rounded-full font-bold text-xs">{item.badge}</span>}
